// Test liboqs ML-DSA-65 verify directly with wrong message
#include <stdio.h>
#include <string.h>
#include <stdint.h>
#include <oqs/oqs.h>
...
int main() {
    ...
    printf("Verify (correct): %d (expected 0 = SUCCESS)\n", ret);
    
    // Verify with wrong message
    uint8_t wrong_message[] = "Wrong message for verification!";
    ret = OQS_SIG_ml_dsa_65_verify(wrong_message, strlen((char*)wrong_message), signature, signature_len, public_key);
    printf("Verify (wrong): %d (expected -1 = ERROR)\n", ret);
