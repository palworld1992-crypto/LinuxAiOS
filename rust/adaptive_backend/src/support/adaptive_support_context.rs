//! Adaptive Support Context - Context flags for support tasks

use common::supervisor_support::types::SupportContext;

#[derive(Debug, Clone)]
pub struct AdaptiveSupportContext {
    pub context: SupportContext,
    pub priority: i32,
}

impl AdaptiveSupportContext {
    pub fn new() -> Self {
        Self {
...
impl Default for AdaptiveSupportContext {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_context_creation() -> anyhow::Result<()> {
        let ctx = AdaptiveSupportContext::default();
    ...
    fn test_with_priority() -> anyhow::Result<()> {
        let ctx = AdaptiveSupportContext::default().with_priority(5);
